...
import { executeCommand, startBackgroundProcess, normalizeForMcp } from "./executor.js";
...
function toolError(message, category = 'general', extra = {}) {
...
// ============================================
server.tool("run_command", "Execute a shell command. Supports project shortcuts defined in .grok-terminal.json (e.g. 'build', 'check:fast', 'verify:all').", {
    ...
    if (!check.allowed) {
        ...
        return toolError(check.reason, 'blocked');
    }
    ...
    try {
        ...
        return toolError(message, category);
    }
});
server.tool("start_process", "Start a long-running or background process. Returns a session ID for later inspection.", {
    command: z.string(),
    ...
    try {
        ...
        return toolError(`Failed to start process: ${message}`);
    }
});
server.tool("list_processes", "List all currently tracked background processes.", {}, async () => {
    return {
        content: [{ type: "text", text: JSON.stringify({ processes: processManager.list() }, null, 2) }],
    };
});
server.tool("read_process_output", "Read output from a background process (supports offset for large outputs).", {
    sessionId: z.string(),
...
server.tool("kill_process", "Terminate a background process.", {
    sessionId: z.string(),
...
server.tool("get_config", "Inspect the currently loaded configuration (allowed commands, shortcuts, limits).", {}, async () => {
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
...
server.tool("reload_config", "Reload .grok-terminal.json from disk without restarting the MCP server.", {}, async () => {
...
async function runProjectShortcut(shortcutOrCommand, customTimeout, signal) {
    try {
        ...
        const category = message.toLowerCase().includes('timeout') ? 'timeout' : 'execution';
        return toolError(`Command failed: ${message}`, category, { command: shortcutOrCommand });
    }
}
server.tool("run_build", "Runs the full project build (bash build.sh). Recommended after code changes.", {
    timeout: z.number().optional().describe("Timeout in ms (default 300000 = 5 minutes)")
}, async ({ timeout }) => {
    logger.info("Helper called: run_build");
    return runProjectShortcut("build", timeout);
});
server.tool("run_check_fast", "Runs the fast project check (very useful for quick validation).", {}, async () => {
    logger.info("Helper called: run_check_fast");
    return runProjectShortcut("check:fast");
});
server.tool("run_verify_all", "Runs the full verification suite. Can take several minutes.", {
    timeout: z.number().optional().describe("Timeout in ms (default 600000 = 10 minutes)")
}, async ({ timeout }) => {
    logger.info("Helper called: run_verify_all");
    return runProjectShortcut("verify:all", timeout);
});
server.tool("quick_check", "Runs the absolute fastest possible sanity check for the project.", {}, async () => {
    logger.info("Helper called: quick_check");
    return runProjectShortcut("quick-check");
});
// ============================================
...
// --- git_status ---
server.tool("git_status", "Shows a clean, compact git status (branch + changes). Works in any git repository.", {}, async () => {
    ...
    try {
        ...
        return toolError(err?.message || "Failed to get git status", "execution");
    }
});
// --- list_scripts ---
server.tool("list_scripts", "Lists all available scripts from package.json (npm/yarn/pnpm/bun). Very useful before using run_script.", {}, async () => {
...
// --- run_script ---
server.tool("run_script", "Runs a script from package.json using the correct package manager (auto-detects npm/yarn/pnpm/bun). Safer than raw run_command.", {
    ...
    try {
        ...
        return toolError(err?.message || `Failed to run script "${name}"`, "execution");
    }
});
// --- project_info ---
server.tool("project_info", "Gives a quick overview of the current project (name, version, package manager, git remote, node version, etc.). Useful for orientation.", {}, async () => {
